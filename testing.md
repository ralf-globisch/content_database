...
- SSM session-manager-plugin installed locally:
  ...
  ```
- Add this block to `~/.ssh/config` (routes all `ssh i-0...` calls through SSM):
  ```
  Host i-*
      User ubuntu
...
# Instance profile
aws iam create-instance-profile --instance-profile-name content-catalogue-ec2
...
```bash
# Stop (preserves EBS, restartable):
make ec2-stop EC2_HOST=$EC2_HOST EC2_INSTANCE_ID=$INSTANCE_ID

# Or terminate permanently:
make ec2-terminate EC2_HOST=$EC2_HOST EC2_INSTANCE_ID=$INSTANCE_ID
```

Both targets automatically run `db-pull` before acting.
